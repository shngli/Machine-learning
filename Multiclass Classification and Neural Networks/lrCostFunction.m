...
% there're many possible vectorized solutions, but one solution looks like:
%   grad = (unregularized gradient for logistic regression)
%   temp = theta; 
%   temp(1) = 0;   % because we don't add anything for j = 0  
%   grad = grad + YOUR_CODE_HERE (using the temp variable)

%see equation on page 4 of ex3.pdf
temp = theta; 
temp(1) = 0; 
J = -sum(y.*log(sigmoid(X*theta))+(1-y).*log(1-sigmoid(X*theta)))/m + lambda*sum(temp.*temp)/(2*m);
